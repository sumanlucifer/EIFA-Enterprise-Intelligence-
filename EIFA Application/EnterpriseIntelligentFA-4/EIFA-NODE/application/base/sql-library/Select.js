...
function SelectQuery(Dialect, opts) {
	...
	var proto = {
		select: function (fields) {
			if (fields) {
				...
				sql.from[sql.from.length - 1].select = sql.from[sql.from.length - 1].select.concat(
					Array.isArray(fields) ? fields : Array.prototype.slice.apply(arguments)
				);
			}
			return this;
		},
		...
		as: function (alias) {
			...
			if (sql.from[idx].select.length) {
				...
				if (typeof sql.from[idx].select[idx2] == "string") {
					sql.from[idx].select[idx2] = {
						c: sql.from[idx].select[idx2]
					};
				}
		...
		having: function () {
			sql.having = Array.prototype.slice.apply(arguments);
			return this;
		},
		...
		build: function () {
			var query = [],
				tmp, from, i, ii, j, ord, str, select_all = true;
			var having = [];
			...
			for (i = 0; i < sql.from.length; i++) {
				...
				for (j = 0; j < sql.from[i].select.length; j++) {
					if (typeof sql.from[i].select[j] == "string" && (sql.from[i].select[j].indexOf(' as ') > -1 || sql.from[i].select[j].indexOf(' AS ') >
							-1)) {
						tmp.push(sql.from[i].select[j]);
					} else {
						...
						if (typeof sql.from[i].select[j] == "object") {
							...
							if (!sql.from[i].select[j].f && sql.from[i].select[j].c) {
								if (sql.from.length == 1) {
									if (sql.from[i].select[j].c.indexOf('/') > -1) {
										tmp.push(sql.from[i].select[j].c);
									} else {
										tmp.push(Dialect.escapeId(sql.from[i].select[j].c));
									}
								} else {
									if (sql.from[i].select[j].c.indexOf('/') > -1) {
										tmp.push(sql.from[i].a, sql.from[i].select[j].c);
									} else {
							...
							if (sql.from[i].select[j].having) {
								//	console.log('VVVVVVVV', JSON.stringify(sql));
			...
			for (i = 0; i < sql.having.length; i++) {
				query.push((i === 0 ? "HAVING" : "AND") + sql.having[i]);
				}
			}
